...
from .models import Indicador, Metrica, HistorialMetrica, Tipo, Ambito
...
def editar_indicadores(request, id):
    ...
    if request.method == 'POST':
        ...
        if formulario2.is_valid():                
            if iferror(request, 'descripcion'):
                indicador.descripcion = request.POST['descripcion']
            if iferror(request, 'ambito') and iferror(request, 'tipo'):
                indicador.ambito = Ambito.objects.get(id=int(request.POST['ambito']))    
                indicador.tipo = Tipo.objects.get(id=int(request.POST['tipo']))  
            indicador.save()
            formulario2.save()  
        return redirect('indicadores')
    
    return render(request, 'indicadores/editar.html', {'form': formulario2, 'indicador':indicador})
